...
public class AuthorizationServerConfig {
    ...
    public SecurityFilterChain asSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        OAuth2AuthorizationServerConfiguration.applyDefaultSecurity(http);	// Habilitar Authorization server
        ...
        // Configurar token (para trabalhar no formato jwt)
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
        return context -> {
            ...
            if (context.getTokenType().getValue().equals("access_token")) {
                // @formatter:off
                context.getClaims()
                        .claim("authorities", authorities)			// Pode ser adicionado informações no token
    ...
    // De codificador do token
    @Bean
    public JwtDecoder jwtDecoder(JWKSource<SecurityContext> jwkSource) {
        return OAuth2AuthorizationServerConfiguration.jwtDecoder(jwkSource);
    }

    // Faz a configuração do algoritmo do RSA (gera a chave para embaralhar com o token)
    @Bean
